...
const normalizeSharedLiveActivityOptions = (options) => {
    if (!options)
        return undefined;
 ...
 * ```
 */
export const useLiveActivity = (variants, options) => {
    ...
    const end = useCallback(async (options) => {
        if (!targetId) {
            return;
        }
    ...
    useEffect(() => {
        ...
        const subscription = addVoltraListener('stateChange', (event) => {
            if (event.activityName !== targetId)
                return;
 ...
 * ```
 */
export const startLiveActivity = async (variants, options) => {
 ...
 * ```
 */
export const updateLiveActivity = async (targetId, variants, options) => {
    if (!assertRunningOnApple())
        return Promise.resolve();
    const payload = renderLiveActivityToString(variants);
    const normalizedSharedOptions = normalizeSharedLiveActivityOptions(options);
 ...
 * ```
 */
export const isLiveActivityActive = (activityName) => {
    if (!assertRunningOnApple())
        return false;
