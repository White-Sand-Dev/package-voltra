 ...
 * ```
 */
export async function reloadLiveActivities(activityNames) {
    if (!assertIOS('reloadLiveActivities'))
        return;
    try {
 ...
 * ```
 */
export async function clearPreloadedImages(keys) {
    if (!assertIOS('clearPreloadedImages'))
        return;
    try {
